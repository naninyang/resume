...
export default function Activity() {
  ...
  const handleAddSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(`/api/activity/${activity.id}`, activity, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const [activityEdit, setFormData] = useState({ ...activity });
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`/api/activity/${activity.id}`, activityEdit, { headers: { Authorization: `Bearer ${token}` } });
