...
export default function Essay() {
  ...
  const saveEssay = async (e) => {
    ...
    const token = localStorage.getItem('token');
    const method = essay.id ? 'put' : 'post';
    const url = essay.id ? `/api/essay/${essay.id}` : '/api/essay';

    try {
      await axios[method](url, essay, { headers: { 'Authorization': `Bearer ${token}` } });
      toast.success('자기소개서가 성공적으로 갱신되었습니다', {
  ...
  useEffect(() => {
    async function fetchAvatarData() {
      try {
        const token = localStorage.getItem('token');
        const response = await axios.get('/api/essay/upload', { headers: { 'Authorization': `Bearer ${token}` } });
  ...
  const onDrop = useCallback(async (acceptedFiles) => {
    ...
    try {
      ...
      if (response.data && response.data.avatar_path) {
        setEssay(prev => ({ ...prev, avatar_path: response.data.avatar_path, avatar_name: response.data.avatar_name, avatar_type: response.data.avatar_type }));
      }
    } catch (error) {
                    ...
                    <div>
                      <FieldGroup>
                        <input
                          ...
                          onChange={(e) => setEssay(prev => ({ ...prev, eng_name: e.target.value }))}
                          required
                        />
                      ...
                      <FieldGroup>
                        <input
                          ...
                          onChange={(e) => setEssay(prev => ({ ...prev, eng_occupation: e.target.value }))}
                          required
                        />
                    ...
                    <div>
                      <FieldGroup className='content-editable'>
                        <TextareaAutosize
                          ...
                          value={essay?.bio || ''}
                          onChange={(e) => setEssay(prev => ({ ...prev, bio: e.target.value }))}
                          minRows={3}
                          required
                        />
                  ...
                  <CheckboxGroup>
                    <div>
                      <input
                        ...
                        name='show'
                        id='show'
                        checked={essay?.show || false}
                        onChange={(e) => setEssay(prev => ({ ...prev, show: e.target.checked }))}
                      />
